...
Agri-SCM Intelligence 프로젝트는 단순한 예측 모델 개발이 아닌,
데이터 수집 → 정제 → 모델 설계 → API 구조 → 서비스 아키텍처 → 시장 구조 분석까지
공급망 예측 플랫폼 전체를 다루는 프로젝트였습니다.
이 과정에서 다음과 같은 중요한 배움을 얻었습니다.

✔ 1. 공급망(SCM) 산업의 구조적 난제를 이해

프로젝트를 진행하며 아래와 같은 산업적 현실을 깊게 이해하게 되었습니다.

농산물 공급량은 기후·토양·병해충 등 외부 변수의 영향력이 매우 큼

데이터의 시간 지연, 품목별 수확 시기 차이, 지역 간 데이터 표준화 부족 등
산업 고유의 복잡한 문제를 해결해야 정확한 예측이 가능함

AI는 인간의 직관을 단순 자동화하는 것이 아니라,
산업의 불확실성을 정량화하고 의사결정을 돕는 도구라는 사실을 명확히 이해

✔ 2. 기술 PM으로서의 조율 능력 성장

팀 내에서 모델링, 백엔드, 서비스 기획을 연결하는 기술 PM 역할을 수행하며 다음을 배웠습니다.

서로 다른 분야의 팀원 간 의견 충돌을 기술 구조로 조율하는 경험

“왜 이 구조여야 하는가?”를 논리적·기술적으로 설명하는 능력

프로젝트의 타임라인 관리, 역할 분배, 우선순위 설정 능력 강화

기술적 의사결정이 모델 성능뿐 아니라 비즈니스 모델과 직결됨을 체감
